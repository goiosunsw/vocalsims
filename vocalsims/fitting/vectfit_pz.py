...
def vectfit_step(f, s, poles, weights=None):
    """
    ...
    A = A * np.outer(weights, np.ones(2*N))
...
def calculate_residues(f, s, poles, rcond=-1, weights=None):
    ...
    A = A * np.outer(weights, np.ones(N))
    # Solve Ax == b using pseudo-inverse
    b = f * weights
    # Solve Ax == b using pseudo-inverse
    b = f
...
def vectfit_auto(f, s, n_poles=10, n_iter=10, show=False,
                 inc_real=False, loss_ratio=1e-2, rcond=-1, track_poles=False,init_resonances=None, weights=None):
    w = imag(s)
    ...
    for _ in range(n_iter):
        poles = vectfit_step(f, s, poles, weights=weights)
        poles_list.append(poles)

    residues = calculate_residues(f, s, poles, rcond=rcond, weights=weights)

    if track_poles:
        return poles, residues, np.array(poles_list)

    print_params(poles, residues, 0, 0)
    return poles, residues 

def poleres2pz(poles,residues, maxsteps=500):
    """
